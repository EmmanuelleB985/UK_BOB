...
class EntropyAdaptiveLayerNorm(nn.Module):
    def __init__(
        self, normalized_shape, eps=1e-5, elementwise_affine=True, tune_affine=True
    ):
        super().__init__()
...
class EntropyAdaptiveInstanceNorm3d(nn.Module):
    def __init__(
        self, num_features, eps=1e-5, momentum=0.1, affine=True, tune_affine=True
    ):
        super().__init__()
        self.inst_norm = nn.InstanceNorm3d(
    ...
    def forward(self, x):
        if self.training and self.tune_affine:
            ...
            return (
                self.weight[None, :, None, None, None] * x_norm
...
class EntropyAdaptiveGroupNorm(nn.Module):
    def __init__(
        self, num_groups, num_channels, eps=1e-5, affine=True, tune_affine=True
    ):
        super().__init__()
    ...
    def __init__(self, original_model, num_tta_passes=3, lr=0.01):
        ...
        for m in self.model.modules():
            if isinstance(
                m,
                (
                    EntropyAdaptiveLayerNorm,
                    EntropyAdaptiveInstanceNorm3d,
                    EntropyAdaptiveGroupNorm,
                ),
            ):
...
def create_comparison_gif(
    scan, pred, gt, output_path, num_frames=100, downsample=2, duration=0.5
):
    """Create annotated comparison GIF with animation"""
    try:
        # Validate dimensions
        assert (
            scan.shape == pred.shape == gt.shape
        ), f"Dimension mismatch: Scan{scan.shape} Pred{pred.shape} GT{gt.shape}"

        # Preprocess scan
        scan = exposure.rescale_intensity(
            scan, in_range=(np.percentile(scan, 1), np.percentile(scan, 99))
        )
        scan = (scan * 255).astype(np.uint8)

        z_size = scan.shape[-1]
        z_indices = np.clip(
            np.linspace(0, z_size - 1, num_frames).astype(int), 0, z_size - 1
        )
        ...
        for z in z_indices:
            ...
            try:
                ...
                pred_mask = np.ma.masked_where(
                    np.rot90(pred[:, :, z] == 0, k=1), np.rot90(pred[:, :, z], k=1)
                )
                ...
                gt_mask = np.ma.masked_where(
                    np.rot90(gt[:, :, z] == 0, k=1), np.rot90(gt[:, :, z], k=1)
                )
        ...
        if frames:
            imageio.mimsave(
                output_path, frames, duration=duration, fps=10, subrectangles=True
            )
...
def main():
    ...
    model.load_state_dict(
        torch.load(os.path.join(args.pretrained_dir, args.pretrained_model_name))[
            "state_dict"
        ]
    )
    ...
    with torch.no_grad():
        ...
        for batch in val_loader:
            ...
            if args.generate_gifs:
                try:
                    ...
                    gif_path = os.path.join(
                        gif_dir, f"{os.path.splitext(img_name)[0]}_comparison.gif"
                    )
            ...
            # Calculate Dice
            dice_scores = [
                dice(val_outputs == i, val_labels == i) for i in range(1, 14)
            ]
