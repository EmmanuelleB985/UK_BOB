...
parser.add_argument(
    "--infer_overlap", default=0.6, type=float, help="sliding window inference overlap"
)
parser.add_argument(
    "--in_channels", default=4, type=int, help="number of input channels"
)
parser.add_argument(
...
def main():
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader):
            ...
            nib.save(
                nib.Nifti1Image(seg_out.astype(np.uint8), affine),
            ...
            for ch_idx, modality in enumerate(["t1", "t1ce", "t2", "flair"]):
                ...
                nib.save(
                    ...
                    os.path.join(
                        output_directory, f"BraTS2023_{num}_{modality}.nii.gz"
                    ),
                )
