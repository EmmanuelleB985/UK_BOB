...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--max_epochs", default=5000, type=int, help="Maximum number of training epochs"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--momentum", default=0.99, type=float, help="Momentum factor for SGD optimizer"
    )
    parser.add_argument(
        "--noamp",
        action="store_true",
    ...
    parser.add_argument(
        "--squared_dice", action="store_true", help="Use squared Dice loss formulation"
    )
    parser.add_argument(
        "--smooth_dr",
        default=1e-6,
...
def setup_optimizer_and_scheduler(
    ...
    if args.lrschedule == "warmup_cosine":
        print(
            f"Using warmup cosine annealing scheduler with {args.warmup_epochs} warmup epochs"
        )
...
def main_worker(gpu: int, args: argparse.Namespace) -> float:
    ...
    if args.distributed:
        ...
        print(
            f"Initialized process group: rank={args.rank}, world_size={args.world_size}"
        )
