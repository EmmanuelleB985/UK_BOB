...
from monai.data import CacheDataset, SmartCacheDataset, load_decathlon_datalist
...
class AbdomenSliceSelector(MapTransform):
    ...
    def __call__(self, data: Dict) -> Dict:
        ...
        if len(selected_indices) == 0:
            print(
                f"Warning: No slices meet annotation threshold {self.min_annotation}, "
                f"using all {label_arr.shape[3]} slices"
            )
            selected_indices = np.arange(label_arr.shape[3])
        else:
            print(
                f"Selected {len(selected_indices)}/{label_arr.shape[3]} slices "
...
class DistributedSampler(Sampler):
    ...
    def __init__(
        ...
        print(
            f"Rank {self.rank}: Handling {self.valid_length} samples "
...
def get_loader(args: Any) -> Tuple[DataLoader, DataLoader]:
    ...
    val_datalist = load_decathlon_datalist(
        json_path, True, "validation", base_dir=data_dir
    )

    print(
        f"Found {len(datalist)} training samples and {len(val_datalist)} validation samples"
    )
    ...
    if args.distributed:
        train_sampler = DistributedSampler(
            dataset=train_ds, shuffle=True, make_even=True
        )
        val_sampler = DistributedSampler(dataset=val_ds, shuffle=False, make_even=False)
...
def create_data_splits(
    ...
    dataset_json = {
        ...
        "labels": {str(i): f"organ_{i}" for i in range(72)},
        "tensorImageSize": "3D",
    }
